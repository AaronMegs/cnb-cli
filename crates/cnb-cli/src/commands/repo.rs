...
fn visibility_to_level(s: &str) -> i64 {
    match s {
        "public" => 0,
        "internal" => 10,
        _ => 20, // "private" or anything else → safest default
    }
}
...
async fn create(ctx: &mut Context, args: CreateArgs) -> Result<(), CliError> {
    // Split slug into owner-path and final name.
    let (slug, name) = args
        .slug
        .rsplit_once('/')
    ...
    let body = repos::CreateRepoBody {
        name: name.to_owned(),
        description: args.description,
        visibility_level: Some(visibility_to_level(&args.visibility)),
        default_branch: args.default_branch,
    };
    let client = ctx.api()?;
    let v = repos::create(client, slug, &body).await?;
    let path = v
        .get("path")
        .and_then(Value::as_str)
...
async fn fork(ctx: &mut Context, args: ForkArgs) -> Result<(), CliError> {
    let repo = ctx.resolve_repo(args.repo.as_deref())?;
    let client = ctx.api()?;
    let items = repos::list_forks(client, &repo).await?;
...
async fn delete(ctx: &mut Context, args: DeleteArgs) -> Result<(), CliError> {
    ...
    let client = ctx.api()?;
    let _ = repos::delete(client, &repo).await?;
...
async fn edit(ctx: &mut Context, args: EditArgs) -> Result<(), CliError> {
    let repo = ctx.resolve_repo(args.repo.as_deref())?;
    if args.name.is_none() && args.description.is_none() && args.default_branch.is_none() {
        return Err(CliError::BadArgs(
            "nothing to edit — pass at least one of --name/--description/--default-branch".into(),
        ));
    }
    let body = repos::EditRepoBody {
        name: args.name,
        description: args.description,
        default_branch: args.default_branch,
    };
    let client = ctx.api()?;
    let _ = repos::edit(client, &repo, &body).await?;
...
async fn archive(ctx: &mut Context, args: ArchiveArgs, unarchive: bool) -> Result<(), CliError> {
    ...
    let client = ctx.api()?;
    if unarchive {
        let _ = repos::unarchive(client, &repo).await?;
        eprintln!("✓ Unarchived {repo}");
    } else {
        let _ = repos::archive(client, &repo).await?;
...
async fn transfer(ctx: &mut Context, args: TransferArgs) -> Result<(), CliError> {
    ...
    let client = ctx.api()?;
    let _ = repos::transfer(client, &repo, &args.to).await?;
...
async fn set_visibility(ctx: &mut Context, args: SetVisibilityArgs) -> Result<(), CliError> {
    ...
    let client = ctx.api()?;
    let _ = repos::set_visibility(client, &repo, level).await?;
