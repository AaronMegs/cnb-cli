...
- [docs/cnb-pipeline.md](docs/cnb-pipeline.md) — `.cnb.yml` design notes: how the cnb-platform pipeline maps onto (and intentionally diverges from) the GitHub Actions workflows in `.github/workflows/`.
...
| **SDK-2** | ✅ done    | Phase 2 fully complete after the cnb 0.2.2 follow-up: every CLI verb routes through the typed SDK and the local `cnb-api` crate has been retired entirely (workspace 7 → 6 crates). The two flows the SDK doesn't model — `cnb api` raw passthrough and `cnb issue --attach` multipart upload — now live in `cnb-cli::http` and ride on top of `client.http().reqwest_client()`. See [`docs/sdk-issues.md`](docs/sdk-issues.md) for 19 tracked upstream issues, [`docs/sdk-0.2.2-upgrade.md`](docs/sdk-0.2.2-upgrade.md) for the upstream-fix matrix, and [`docs/known-gaps.md`](docs/known-gaps.md) for remaining external-dependency items. |
