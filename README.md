...
| **SDK-2** | in progress | Phase 2: every command group is SDK-backed for read paths, most for write paths. **Done**: `cnb repo` (11/11, incl. `pin/unpin/list-pinned/contributors` on step 2.10), `cnb label` (full), `cnb release` (full), `cnb build` (full), `cnb workspace` (full), `cnb registry` (full), `cnb mission` (full), `cnb org` (full), `cnb search`; **read-only SDK port** for `cnb issue` + `cnb pr`. **Residue** on the `cnb-api` facade: `cnb issue/pr` write paths (step 2.11), `cnb auth login` token validation (Phase 1 hold-over), `cnb api` raw passthrough (blocked by SDK-I14). See `docs/sdk-issues.md` for 18 tracked upstream issues. |
