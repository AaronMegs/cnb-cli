...
| **SDK-2** | in progress | Phase 2: port commands to SDK module-by-module. Done: `cnb repo view/list`, `cnb issue view/list`, `cnb pr view/list`, **`cnb label` (full set, first complete group)**. Next: `cnb repo` writes, then `release/build/workspace`, then `registry/mission/org` |
