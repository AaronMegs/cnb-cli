...
| **SDK-2** | functionally complete | Phase 2: every command group the SDK covers is now typed-SDK-backed. **Done: `cnb repo` (10/14), `cnb issue`, `cnb pr`, `cnb label`, `cnb release`, `cnb build`, `cnb workspace`, `cnb registry`, `cnb mission`, `cnb org`, `cnb search`.** Residue on the `cnb-api` facade: `cnb auth login` token validation (Phase 1), `cnb api` raw passthrough (blocked by SDK-I14 JSON-only transport), `cnb repo pin/contributors` (SDK hasn't exposed those endpoints yet). See `docs/sdk-issues.md` for 16 tracked upstream issues. |
